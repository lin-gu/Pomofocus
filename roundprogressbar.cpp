...
RoundProgressBar::RoundProgressBar(QWidget *parent) :
    QWidget(parent),_min(0),_max(100),_value(0),_precision(0)
{
    //设置初始角度，顺时针逆时针
    setdefault(90,true);
    //设置默认外圈宽度
...
//设置时钟上标题信息
void RoundProgressBar::setToptitle(QString toptitle)
{
    _toptitle = toptitle;
}

//设置时钟副标题信息
void RoundProgressBar::setSubtitle(QString subtitle)
{
    _subtitle = subtitle;
}
